...
- 跨主题记忆共享开关：写入官方 `ovcli.conf` 的 `plugin.recallPeerScope`（默认保持官方行为 `all` 允许跨主题共享；关闭为 `actor`，自动召回仅限当前项目 peer 的记忆），保存后**自动重新加载**官方记忆插件使配置即刻生效（卡片文案会提前提示重载及其副作用；自动重载未完成时回退为手动重载按钮）；用户级画像注入与共享资源不受该开关影响；`.openviking/config.json` 等 workspace 配置由用户手工管理，本插件不读写；
