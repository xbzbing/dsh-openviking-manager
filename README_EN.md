...
- Cross-topic memory sharing switch: writes the official `plugin.recallPeerScope` key in `ovcli.conf` (the default keeps OpenViking's stock `all` behaviour; turning it off writes `actor`, restricting automatic recall to the current project's peer memories) and offers a "Restart official memory plugin" button so the change applies immediately. The user-level profile injection and shared resources are not affected by this switch.
...
- The sharing switch only writes the officially declared `plugin.recallPeerScope` key and preserves every unknown key in `ovcli.conf`. "Restart official memory plugin" reloads the official plugin through the host's public Cordis mechanisms so it re-reads its configuration; it never modifies official code. Side effects: one final commit/archive pass over open sessions and a brief MCP tool rebuild. When the official plugin is not loaded, the page asks you to restart the DSH instance manually. The `OPENVIKING_RECALL_PEER_SCOPE` environment variable outranks every file and shows an override warning on the page.
